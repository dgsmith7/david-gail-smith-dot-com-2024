...
async function buildStaticSite() {
  ...
  try {
    const indexTemplate = fs.readFileSync(
      path.join(__dirname, "views", "index-clean.ejs"),
      "utf8"
    );
    
    const headerTemplate = fs.readFileSync(
      path.join(__dirname, "views", "header-clean.ejs"),
      "utf8"
    );
    
    const footerTemplate = fs.readFileSync(
      path.join(__dirname, "views", "footer-clean.ejs"),
      "utf8"
    );

    // First, manually replace includes in the main template
    let templateWithIncludes = indexTemplate;
    templateWithIncludes = templateWithIncludes.replace(/<%- include\(['"`]header-clean['"`]\); %>/g, headerTemplate);
    templateWithIncludes = templateWithIncludes.replace(/<%- include\(['"`]footer-clean['"`]\); %>/g, footerTemplate);

    const html = ejs.render(templateWithIncludes, {
      projects: projects,
      categories: categories,
    }, {
      filename: path.join(__dirname, "views", "index-clean.ejs")
    });
